...
def BuildDag(BLOSUM62, indel_penalty, s, t):
    n = len(s)
    m = len(t)
    ...
    for i in range(0, n):
        for j in range(0, m):
            ...
            dag[(i, j)].append(((i+1, j+1), BLOSUM62[s[i]][t[j]]))
    ...
    for d in range(0, n):
        dag[(d, m)].append(((n, m), 0))
    
    return dag


def LongestPath(s: str, t: str, indel_penalty:int, BLOSUM62) -> tuple[int, list[int]]:

    dag = BuildDag(BLOSUM62, indel_penalty, s, t)
...
def FittingAlignment(s: str, t: str, indel_penalty:int, BLOSUM62):

    tup = LongestPath(s, t, indel_penalty, BLOSUM62)
...
file = open('Chapter4\dataset_865450_5.txt')
s = file.readline().strip()
t = file.readline().strip()

ip = 1

result = FittingAlignment(s, t, ip, BLOSUM62)
